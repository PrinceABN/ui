...
#include "../../framework/dependency/dep.hpp"
...
IDirect3DTexture9* pattern = nullptr;
IDirect3DTexture9* oneicon = nullptr;
IDirect3DTexture9* twoicon = nullptr;
IDirect3DTexture9* threeicon = nullptr;
IDirect3DTexture9* fouricon = nullptr;
IDirect3DTexture9* fiveicon = nullptr;
...
sdk::c_lresult CALLBACK wnd_handle( sdk::c_hwnd w, UINT msg, WPARAM wp, LPARAM lp ) {
	if ( msg == WM_MOUSEMOVE )
		dependency::dep_input->set_mouse_position( sdk::math::vec2_t( GET_X_LPARAM( lp ), GET_Y_LPARAM( lp ) ) );

	if ( msg == WM_MOUSEWHEEL )
		dependency::dep_input->set_mouse_wheel( GET_WHEEL_DELTA_WPARAM( wp ) / WHEEL_DELTA );
...
sdk::c_function i::hooks::impl::init_imgui( sdk::c_hwnd w, sdk::c_dev* device ) {
	...
	if ( pattern == nullptr )
		D3DXCreateTextureFromFileInMemoryEx( device, rpatern, sizeof( rpatern ), 1920, 1080, D3DX_DEFAULT, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &pattern );

	if ( oneicon == nullptr )
		D3DXCreateTextureFromFileInMemoryEx( device, &oneiconz, sizeof( oneiconz ), 22, 20, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &oneicon );
	if ( twoicon == nullptr )
		D3DXCreateTextureFromFileInMemoryEx( device, &twoiconz, sizeof( twoiconz ), 22, 20, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &twoicon );
	if ( threeicon == nullptr )
		D3DXCreateTextureFromFileInMemoryEx( device, &threeiconz, sizeof( threeiconz ), 22, 20, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &threeicon );
	if ( fouricon == nullptr )
		D3DXCreateTextureFromFileInMemoryEx( device, &fouriconz, sizeof( fouriconz ), 22, 20, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &fouricon );
	if ( fiveicon == nullptr )
		D3DXCreateTextureFromFileInMemoryEx( device, &fiveiconz, sizeof( fiveiconz ), 22, 20, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &fiveicon );
